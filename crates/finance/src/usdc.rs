...
use crate::{HasZero, float_integer_string};
...
impl Usdc {
    ...
    pub fn from_cents(cents: i64) -> Option<Self> {
        ...
        (cents_float / float!(100)).ok().map(Self)
    }
    ...
    pub fn to_cents(self) -> Result<i64, UsdcToCentsError> {
        ...
        let integer_str = float_integer_string(scaled)?;
        integer_str
            .parse::<i64>()
            .map_err(|_| UsdcToCentsError::Overflow(self))
    }

    /// Splits this amount into a value truncated to `decimals` places and
...
mod tests {
    ...
    #[test]
    fn truncate_to_decimals_returns_value_and_discarded_dust() {
        let amount = Usdc::new(float!(100.1234567));

        let (truncated, dust) = amount.truncate_to_decimals(6).unwrap();

        assert!(truncated.eq(&Usdc::new(float!(100.123456))).unwrap());
        assert!(dust.eq(&Usdc::new(float!(0.0000007))).unwrap());
    }
