...
//!
//! Generated expressions name `rain_math_float` and `alloy_primitives`
//! directly. Consumers must therefore declare both packages alongside
//! `st0x-float-macro`, using the exact versions and pinned Float revision shown
//! in the repository README. The external-consumer integration test locks down
//! that documented dependency contract.
...
/// # Panics
///
/// Panics if the internally generated Rust source cannot be parsed into a token stream.
#[proc_macro]
